            ...
            <template
              ...
              <div
                ...
                <div class="flex items-center justify-between px-4 py-3 border-b border-slate-800">
                  ...
                  <button v-else-if="showSettingsPanel" @click="closeSettingsPanel"
                    class="w-7 h-7 rounded-full bg-slate-800/80 text-slate-200 flex items-center justify-center text-xs border border-slate-600/70">
                    ✕
                  </button>
                      ...
                      <div class="space-y-2">
                        <label class="flex items-center justify-between text-xs">
                          ...
                          <button @click="snippetAudioOn = !snippetAudioOn"
                            ...
                            <span
                              class="inline-block h-4 w-4 transform bg-white rounded-full shadow transition-transform"
                    ...
                    <div>
                      ...
                      <label class="flex items-center justify-between text-xs">
                        ...
                        <button @click="/* placeholder futuro */ 0"
                          ...
                          <span
                            class="inline-block h-4 w-4 transform bg-white rounded-full shadow translate-x-0"></span>
                        </button>
                      </label>
                    </div>
                  </div>
                ...
                <!-- Corpo: privacy -->
                <template v-else-if="showPrivacyPanel">
...
export default defineComponent({
  ...
  methods: {
    ...
    openSettings() {
      // apre il pannello impostazioni sopra l'avatar / chat, nella stessa posizione della modalità testo
      this.snippetMenuOpen = false;
      this.showPrivacyPanel = false;
      this.snippetTextMode = false;
      this.showSettingsPanel = true;
      // Come in modalità testo, spengo temporaneamente il video/avatar
      try {
        if (this.heygenVideo) {
          this.heygenVideo.muted = true;
          this.heygenVideo.pause?.();
    ...
    openPrivacyPanel() {
      // apre il pannello con le informazioni sulla privacy nella stessa posizione della modalità testo
      this.snippetMenuOpen = false;
      this.showSettingsPanel = false;
      this.snippetTextMode = false;
    ...
    async onMicClick() {
      // Se l'avatar sta parlando, il bottone si comporta da STOP e interrompe il parlato
      if (this.isSpeaking) {
        try {
          console.log("SPEECH: interrupt requested by user");
    ...
    async heygenSendRepeat(text) {
      try {
        ...
        // L'avatar inizia a parlare: aggiorna lo stato UI
        ...
        // la usiamo per sapere fino a quando considerare l'avatar "in parlato".
        try {
    ...
    interruptAvatarSpeech() {
      try {
        ...
        // Interrompi l'audio/video dell'avatar (la sessione resta attiva)
        if (this.heygenVideo) {
          try {
            this.heygenVideo.pause?.();
          } catch { }
