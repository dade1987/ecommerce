    ...
    <body>
        ...
        if ($status == AUTH_LOGGED) {
            ...
            if (license_has($user, "sede_centrale"))
                $risultati_ddt = $db_fatture->query("SELECT * FROM ddt WHERE tipo='DDT' ORDER BY id DESC;");
            else
                $risultati_ddt = $db_fatture->query("SELECT * FROM ddt WHERE tipo='DDT' AND negozio='" . $user['nome_negozio'] . "' ORDER BY id DESC;;");
            ...
            for ($i = 0; $ddt = $risultati_ddt->fetch_assoc(); $i++) {
                ...
                echo "<a href=./visualizza_ddt.php?id=" . $ddt['id'] . ">DDT numero " . $ddt['numero'] . " del " . $ddt['data'] . " per " . $ddt['negozio'] . "</a></td></tr>";
            }
            echo "</table>";
