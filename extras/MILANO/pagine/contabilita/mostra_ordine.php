<html>
    ...
    <body>
        ...
        if ($status == AUTH_LOGGED) {
            ...
            $ordine = $db_fatture->query("SELECT * FROM db_ordini WHERE id='" . htmlentities($_GET['id']) . "'");

            $ordine = $ordine->fetch_assoc();

            if ($status == AUTH_LOGGED && license_has($user, "sede_centrale")) {
                    $db_fatture->query("UPDATE db_ordini SET visualizzato=1 WHERE id='" . htmlentities($_GET['id']) . "'");
            }
                ...
                echo $ordine;
                echo "</div>";
                echo "<button class='btn btn-default ' value='Stampa' onClick='stampa();'>STAMPA DOCUMENTO</button>";
            } else
