    ...
    <body>
        ...
        if ($status == AUTH_LOGGED) {
            ...
            if (license_has($user, "sede_centrale")) {
                $query = "SELECT * FROM db_fatture ORDER BY numero_fattura DESC;";
            } else {
                $query = "SELECT * FROM db_fatture WHERE (intestatario = '" . $user['nome_negozio'] . "' OR intestatario = '" . $user['nome_sede_legale'] . "') ORDER BY numero_fattura DESC;";
            }

            $risultato = $db_fatture->query($query);

            echo "<table style='border:1px solid black; background-color:white;'>";
            while ($fatture = $risultato->fetch_assoc()) {
                //var_dump($fatture);
                echo "<tr style='border:1px solid black;'><td style='padding:10px;'>";
                if ($fatture['visualizzato'] == 0) {
                    echo "(NUOVA!) ";
                }
                echo "<a href='./mostra_fattura.php?id=" . $fatture['id'] . "'>FATTURA numero " . $fatture['numero_fattura'] . " del " . $fatture['data'] . " di " . $fatture['intestatario'] . "</a></td></tr>";
            }
            echo "</table>";
