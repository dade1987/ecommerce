        ...
        if ($status == AUTH_LOGGED && license_has($user, "sede_centrale")) {
            ...
            <div class="container-fluid">
                <div class="row clearfix">
                    <div class="col-md-12">
                        <h1>ORDINE DA VENDUTO</h1>
                    </div>
                </div>
                <div class="row clearfix">
                    <div class="col-md-4">
                        <label>Barcode</label><input class="form-control" type="text" id="barcode" placeholder="Barcode"></input>
                    </div>
                    <div class="col-md-3">
                        <label>Data iniziale</label><input class="form-control" type="date" id="data_da" placeholder="Data iniziale" value="<?php echo date('Y-m-d', strtotime('now-7 day')); ?>"></input>
                    </div>
                    <div class="col-md-3">
                        <label>Data finale</label><input class="form-control" type="date" id="data_a" placeholder="Data finale" value="<?php echo date('Y-m-d', strtotime('now-1 day')); ?>"></input>
                    </div>
                    <div class="col-md-2">
                        <label>Stampa DDT</label><button class="form-control btn btn-success" id="submit">Stampa DDT</button>
                    </div>
                </div>
            ...
            <script>
                ...
                $('#submit').click(function () {
                    ...
                    var posting = $.post('./ddt_ordine_da_venduto_ajax.php', {ordine_raggruppato: ordine_raggruppato},
                    function (data) {
                ...
                function inserisci_articolo() {
                    ...
                    var data_da = $('#data_da').val() + " 00:00:00";
                    var data_a = $('#data_a').val() + " 00:00:00";
                    // Send the data using post
                    var posting = $.post('./ordine_da_venduto_ajax.php',
                            {barcode: barcode, data_da:data_da, data_a:data_a},
                    function (data) {
                        //console.log(data);
                        ...
                        data.forEach(function (obj) {
                            ...
                            options += '<option value="' + obj.negozio + '">' + obj.negozio + ' (Da caricare (venduti): ' + differenza + ')</option>';
                        });
