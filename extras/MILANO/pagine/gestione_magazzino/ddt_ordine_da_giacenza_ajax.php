...
if ($status == AUTH_LOGGED && license_has($user, "sede_centrale")) {
    ...
    foreach ($ordine as $nome_negozio => $array_negozio) {
        ...
        $query_inserimento_ddt = "INSERT INTO ddt (anno,tipo,mittente,numero,data,ddt,negozio,codice_tracciatura) VALUES ('".date('Y')."',\"DDT\",\"" . $user['nome_negozio'] . "\",\"$numero_ddt\",\"" . date('Y-m-d H:i:s', strtotime('now')) . "\",\"" . utf8_encode($ddt) . "\",\"$nome_negozio\",\"N.D.\");";
